/********************************************************
   ...
   2/12/2021 Update by Rich. Add controlTemperature in settings.h to 28 should
   adjust to baseline temp, but should make it so the temperature control is constant if no 
   Temp reading
 ********************************************************/
...
// #include <DS1307.h>  No longer neeed 
#include "RTClib.h"

#include <ArduinoUniqueID.h> //For unique ID
 //Added code in setup to show and display plus make variable. Log file should add uique ID
...
// DS1307:  SDA pin   -> Arduino Digital 19 CHANGED TO 20 for new library
//          SCL pin   -> Arduino Digital 18 CHANGED TO 21 for new library
//DS1307  rtc(20, 21);
//Time  t; //Can this come up
RTC_DS1307 rtc;
DateTime t;
...
File logFile;
File setpoints;
String printdate = "pulchra_slowburn"; // No spaces, dashes, or underscores   **** generated Automaticaly ****
...
String uniqueID="";
  ...
  for(size_t i =UniqueIDsize-3; i<UniqueIDsize; i++){
    ...
    shortUniqueID=shortUniqueID+UniqueID[i];
  }
  lcd.clear();
lcd.setCursor(0, 0);
  lcd.print("Short Unique ID:");
  lcd.setCursor(6, 1);
  lcd.print(shortUniqueID);
  
  Serial.print("Short unique ID: \t");
  Serial.println(shortUniqueID);
...
void loop()
{
 ...
 // ***** INPUT FROM TEMPERATURE SENSORS *****
  ...
  if (10.0 < tempT1 && tempT1 < 80.0)  {//If incoming is less than 10 probably a problem
    T1TempInput = tempT1;} else{ 
  T1TempInput=controlTemperature;} // added 2/12/2021 

  tempT2 = sensors.getTempCByIndex(T2Index) - T2Correction;
  if (10.0 < tempT2 && tempT2 < 80.0)  {//If incoming is less than 10 probably a problem
    T2TempInput = tempT2;} else{ 
  T2TempInput=controlTemperature;} // added 2/12/2021  // added 2/12/2021 probably some way to make this conditional
  tempT3 = sensors.getTempCByIndex(T3Index) - T3Correction;
  if (10.0 < tempT3 && tempT3 < 80.0){ // Above plus probe correction value
    T3TempInput = tempT3;}else{
     T3TempInput=controlTemperature;} 
  

  tempT4 = sensors.getTempCByIndex(T4Index) - T4Correction;
  if (10.0 < tempT4 && tempT4 < 80.0)  { // Above plus probe correction value
    T4TempInput = tempT4;}else{
      T4TempInput=controlTemperature;}
...
{ 
  if (SerialOutCount>100){
    Serial.print("ShortUniqueID,PrintDate,Date,N_ms,Th,Tm,Ts,");
    ...
    if (logFile) {
    logFile.print("ShortUniqueID, PrintDate,Date,N_ms,Th,Tm,Ts,");
  ...
  Serial.print(shortUniqueID), Serial.print(","),Serial.print(printdate), Serial.print(","), Serial.print(getdate()), Serial.print(","), Serial.print(now_ms), Serial.print(","), Serial.print(t.hour(), DEC), Serial.print(","), Serial.print(t.minute(), DEC), Serial.print(","), Serial.print(t.second(), DEC), Serial.print(",");
  ...
  logFile = SD.open("log.txt", FILE_WRITE);
  if (logFile) {
    logFile.print(shortUniqueID), logFile.print(","), logFile.print(printdate), logFile.print(","), logFile.print(getdate()), logFile.print(","), logFile.print(now_ms), logFile.print(","), logFile.print(t.hour(), DEC), logFile.print(","), logFile.print(t.minute(), DEC), logFile.print(","), logFile.print(t.second(), DEC), logFile.print(",");
