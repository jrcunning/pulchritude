...
double controlTemperature=70.1; //Temperature defined by user in case probes aren't reading.

#define CHILLER_OFFSET 0.20
#define TANK1_TEMP_CORRECTION -0.34 // Is a temperature correction for the temp sensor, the program subtracts this from the temp readout e.g. if the sensor reads low, this should be a negative number
#define TANK2_TEMP_CORRECTION -0.25 // Is a temperature correction for the temp sensor, the program subtracts this from the temp readout e.g. if the sensor reads low, this should be a negative number
#define TANK3_TEMP_CORRECTION -0.31 // Is a temperature correction for the temp sensor, the program subtracts this from the temp readout e.g. if the sensor reads low, this should be a negative number
#define TANK4_TEMP_CORRECTION -0.55 // Is a temperature correction for the temp sensor, the program subtracts this from the temp readout e.g. if the sensor reads low, this should be a negative number
